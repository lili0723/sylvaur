...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    description: t("scarves.description"),
  };
}
...
export default async function ScarvesPage({ params }: Props) {
  ...
  const products = getProducts("scarves").filter(p => p.frontmatter?.slug).map((p) => ({
    ...p,
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {products.map((p) => (
              <ProductCard
                ...
                href={`/${locale}/products/scarves/${p.frontmatter.slug}`}
              />
            ))}
          </div>
        ) : (
